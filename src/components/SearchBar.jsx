...
const SearchBar = ({searchQuery, setSearchQuery, setSearchTerm}) => {
  return (
    <div className='mx-auto mt-8 flex max-w-2xl flex-col gap-3 rounded-2xl border border-white/10 bg-white/5 p-2 shadow-2xl backdrop-blur-xl sm:flex-row'>
        ...
        <button id='button' onClick={()=> {if(!searchQuery.trim()) return; setSearchTerm(searchQuery.trim()); setSearchQuery("")}} className='rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 px-6 py-3 font-semibold text-white transition hover:scale-[1.02] hover:shadow-lg'>🔍Search</button>
    </div>
  )
}
